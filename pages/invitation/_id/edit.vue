<template>
  <div class="mx-auto pb-5">
    ...
    <div class="w-full h-custom-insert-employee p-5 lg:overflow-y-auto scr-custom lg:pr-2">
      <FormulateForm
        class="invitation-edit-form nl-transition"
        name="invitation-edit-form"
...
export default {
  ...
  async fetch() {
    await this.$store.dispatch(
      'invitation/loadSingleEditInvitation',
      this.$route.params.id
    );
  },
  created() {
    this.dataInvitation = this.$store.state.invitation.singleEditInvitation;
  },
  methods: {
    sendingData() {
      if (this.cekValid()) {
        ...
        this.$store.dispatch('invitation/updateDataInvitation', data);
        this.isLoading = true;
    ...
    reset() {
      this.$formulate.reset('invitation-edit-form');
      this.resIdObjectIndataInvitation();
      this.btnResetVisible = true;
    },
    cancelReset() {
      this.dataInvitation = this.$store.state.invitation.singleEditInvitation;
...
<style scoped>
.invitation-edit-form::v-deep .formulate-input .formulate-input-element {
  max-width: none;
}
